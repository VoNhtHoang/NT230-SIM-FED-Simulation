...
class Client:
    def __init__(self, client_name, data_train, data_val, data_test, steps_per_epoch, val_steps, test_steps, active_clients_list, num_classes):
    ...
    def init_model(self):
        ...
        loss_func = 'categorical_crossentropy' if self.num_classes > 1 else 'binary_crossentropy'
        activation_func = 'softmax' if self.num_classes >1 else 'sigmoid'
        print(f"Num classe: {self.num_classes}\nLoss Fucntion: {loss_func}\nActivation Function: {activation_func}")
    ...
    def model_fit(self, iteration):
        ...
        if iteration > 1:
            ...
            #         index += 1
            model_path = os.path.join(self.temp_dir, f"global_model_iter_{iteration-1}.keras")
        ...
        self.local_weights[iteration] = []
        self.local_biases[iteration] = []
        for layer in self.model.layers:
            if layer.name.startswith('conv1d') or layer.name.startswith('dense'):
                self.local_weights[iteration].append(layer.get_weights()[0])
                self.local_biases[iteration].append(layer.get_weights()[1])
        del csv_logger, file_path, file_path_model
        gc.collect()
        return None
    ...
    def proc_weights(self, message):
        ...
        body = {
            'weights': self.local_weights[iteration],  
            'biases': self.local_biases[iteration],
            'iter': iteration,
    ...
    def recv_weights(self, message):
        ...
        self.global_weights[iteration], self.global_biases[iteration] = body['weights'], body['biases']
        ...
        del self.local_weights[iteration], self.local_biases[iteration]
        gc.collect()
